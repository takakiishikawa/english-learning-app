...
const PRESET_GRAMMAR_TAGS = [
  "S + là + N（〜は〜です）",
  "形容詞は直接つなぐ",
  "không + V/形容詞（否定）",
  "không phải là + N（名詞否定）",
  "có ... không?（Yes/No疑問）",
  "... phải không?（確認）",
  "ai ...?（誰）",
  "... gì?（何）",
...
const PRESET_PHRASE_TAGS = [
  ...
  "Nói chậm hơn được không?（もう少しゆっくり）",
  "Hôm nay thế nào?（今日どうだった？）",
];
...
export function ViAddModal({
  ...
  const hasInput = text.trim().length > 0 || selectedTags.length > 0;
  ...
  function buildExtractInput(): string {
    const tagBullets = selectedTags.map((t) => `- ${t}`);
    const free = text.trim();
    return [...tagBullets, free].filter(Boolean).join("\n");
  }
  ...
  async function handleExtract() {
    const combined = buildExtractInput();
    if (!combined) return;
    setLoading(true);
    try {
      const res = await fetch("/api/extract", {
        ...
        body: JSON.stringify({ text: combined, language: "vi" }),
      });
  ...
  function renderTagSection(label: string, tags: readonly string[]) {
    return (
          ...
          {tags.map((tag) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "inline-flex items-center rounded-full border px-2.5 py-1 text-xs leading-tight transition-colors",
                  active
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    よく使う文法・フレーズをタップで追加。下の自由入力と合わせて投げられます。
                  </p>
                </div>
                {renderTagSection("文法", PRESET_GRAMMAR_TAGS)}
                {renderTagSection("フレーズ", PRESET_PHRASE_TAGS)}
                {selectedTags.length > 0 && (
                  <div className="flex items-center justify-between rounded-md border border-dashed bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                    <span>{selectedTags.length} 件 選択中</span>
                    <button
                      type="button"
