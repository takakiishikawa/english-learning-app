...
export function GenerateImagesButton({
  ...
  async function callApi(batch: { id: string; name: string }[]) {
    ...
    const rateLimited = results.length > 0 && results.every((r) => r.reason?.includes("429"))
  ...
  async function handleBatchGenerate() {
    ...
    if (cancelledRef.current) {
      toast.info(`キャンセルしました（${totalSuccess}件成功・${totalFail}件失敗）`)
    } else if (totalFail > 0) {
      toast.error(`${items.length}件中 ${totalSuccess}件成功・${totalFail}件失敗`)
    } else {
      toast.success(`${totalSuccess}件すべて生成完了！ページを更新してください`)
    }
  }
    ...
    try {
      for (let i = 0; i < items.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          try { data = JSON.parse(text) } catch { throw new Error(`サーバーエラー (HTTP ${res.status})`) }
  ...
  return (
    <div className="flex items-center gap-2">
      <Button
        ...
        className="gap-2"
      >
        {isRunning ? <Loader2 className="h-4 w-4 animate-spin" /> : <ImageIcon className="h-4 w-4" />}
        {buttonLabel}
      </Button>
      {isRunning && force && (
